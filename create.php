...
?>
				...
				<?php
					if($_SERVER['REQUEST_METHOD'] == "POST"){
						...
						if(empty($st_id) or empty($st_name) or empty($st_pass ) or empty($st_email) or empty($BirthMonth) or empty($BirthDay) or empty($BirthYear) or empty($st_dept) or empty($st_contact) or empty($st_gender) or empty($st_add)){
							...
							$st_pass = md5($st_pass);
							$st_register = $user->crud_registration($st_id,$st_name,$st_pass,$st_email,$bday,$st_dept,$st_contact,$st_gender,$st_add);
							if($st_register){
								echo "<h3 style='color:green;margin:0;padding:0;text-align:center'>Registration Complete !!</h3>";
							}else{
		...
		<form action="" method="post" id="st_form">
			<table>
				...
				<tr>
					<td colspan="2"><input type="submit" name="sub"  style="color:#ddd;background:#454544"value="Register" /></td>
				</tr>
			</table>
		</form>
